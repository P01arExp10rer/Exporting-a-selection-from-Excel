...
{
    public static class ExcelOleDbSearcher
    {
        public static DataTable Search(ExcelSearchSettings settings)
        {
            ...
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                ...
                for (int row = headerRow + 1; row <= endRow; row++)
                {
                    ...
                    bool match = false;
                    if (isExact)
                        match = symbols.Any(sym => string.Equals(cellNorm, sym, StringComparison.OrdinalIgnoreCase));
                    else
                        match = symbols.Any(sym => cellNorm.IndexOf(sym, StringComparison.OrdinalIgnoreCase) >= 0);
